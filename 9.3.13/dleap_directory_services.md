...
The Directory Service is very convenient for use in populating a dynamic role. For example, in the **Assign users to a role** action available on any of the submit buttons on the Stages page, you could use the **Get Related User By Attribute** operation on the Directory Service to look up the manager of the current user to put the manager in an Approval role for that record.
