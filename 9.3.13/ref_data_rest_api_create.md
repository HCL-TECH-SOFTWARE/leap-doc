...
```POST to /secure/org/data/[app_uid]/[form_id]?draft=true``` will save the record as a draft in the Start stage, for the current user

Subsequent POST requests to ```/secure/org/data/[app_uid]/[form_id]?draft=true``` will replace the user’s pre-existing draft, not create a new draft.

Omit ```draft=true``` from POST request to replace a draft with a proper submission.

!!!note
    The id of a record will change each time it is saved as a draft; the record UID will remain the same.
