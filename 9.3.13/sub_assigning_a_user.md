...
You can dynamically add a user to a role as part of a form submission.

You can assign users to roles dynamically using a web service with data, or metadata captured from the form. Assign an approver or reviewer by creating rules based on the data from the form, or by who submitted the form.

!!!note
    This is useful for dynamically reassigning role members while keeping the record in the same stage and permissions.

For example, you can create a workflow application with rules to determine who can send notification by email.

To use the following instructions, you must have user Roles, such as “Manager”, set to **Open**. For more information on setting roles, see [Application and Security overview](se_security_toc.md).
...
4.  In the **Activity Settings** panel, select **Assign users to a role**.
