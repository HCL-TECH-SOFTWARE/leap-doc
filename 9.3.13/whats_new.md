...
- Added new submit activity [Remove users from role](sub_removing_a_user.md)
...
{% if isDominoLeap %}
- Added the config property [disableExpandGroupsInDominoEmail](co_configuration_properties.md#disableexpandgroupsindominoemail)
- Added the [My Tasks](sub_adding_stages_toc.md#my-tasks) page
- Added [primary assignee](sub_adding_stages_toc.md#designating-a-primary-assignee) to workflow
{% endif %}
{% if isLeap %}
...
- Added the new javaScript functions, [getAppDescription](ref_application_object.md#getappdescription) and [getDBId](ref_jsapi_ref_data_objects.md#getdbid)


### Other Changes

- Importing an application with data no longer loads all data into memory. The application data is now streamed and processed one entry at a time.
- Images included in a RichText item are now physically attached when the content is included in the body of an email submit activity.
- Improved the layout of the Files section of the Events page
- The password item has been deprecated and removed from the product
- General fixes and security patches
...
{% endif %}
- Added [Swagger UI](ref_swagger_ui.md) to interact with REST APIs
- Updated [REST API](ref_rest_api_ref.md) definitions to OpenAPI v3
...
- The metadata REST API has been removed, because its content was incorporated into the existing APIs
- General fixes and security patches
