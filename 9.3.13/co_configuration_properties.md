...
### dataImportTransactionSize

When importing an application with data, this property will cause {{shortProductName}} to save after the specified number of records are processed.

**Example:**

``` 
{% if isLeap %}ibm.nitro.NitroConfig.{% endif %}dataImportTransactionSize=1000
```
...
{% if isDominoLeap %}
### disableExpandGroupsInDominoEmail

If *true*, when sending an email, from Domino Leap, to a Domino group, the group will NOT be replaced with all the members of the group.
...
```
disableExpandGroupsInDominoEmail=true
```
