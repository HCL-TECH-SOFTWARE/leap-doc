...
Returns the content of the application description field on the General settings page.
...
**Example**
```javascript
var myAppDesc = app.getAppDescription();
```
...
| appUid        | Optional. The app uid, if not specified the current app uid is used.  Note: must be supplied if passing a value for newTab.|
...
app.openAppPage ('AP_Page1', null, true);
...
app.openForm('F_Form1', null, true);
app.openForm ('F_Form1', 'd1f6eb71-9483-479c-8d47-dd30bd7e9de9', false);
...
| :------------ | :---------- |
| recordUid     | The submitted record uid. |
| formId        | Optional. The form id, if not specified the current form id is used. Note: must be supplied if passing other parameters. |
| appUid        | Optional. The app uid, if not specified the current app uid is used. Note: must be supplied if passing other parameters. |
| newTab        | Optional. True or False.  If True, the record will be opened in a new browser tab. Default is True.|

**Example:**
```javascript
// open in same tab, only supply the record id
app.openRecord('xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx');

// open in a same tab, specify record uid and form id
app.openRecord('xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', 'F_Form2');

// open form in a new tab, specify the record uid and app uid
app.openRecord('xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', null, 'yyyyyyyy-yyyy-yyyy-yyyy-yyyyyyyyyyyy', true);

// open form in new tab, specify record uid, form id and app uid
app.openRecord('xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', 'F_Form2', 'zzzzzzzz-zzzz-zzzz-zzzz-zzzzzzzzzzzz', true);
