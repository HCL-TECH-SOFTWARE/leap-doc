...
```PUT to /secure/org/data/[app_uid]/[form_id]/[record_uid]?draft=true``` to save the record as a draft, for the current user.

Subsequent PUT requests to ```/secure/org/data/[app_uid]/[form_id]/[record_uid]?draft=true``` replace the user’s existing draft rather than creating a new one. Each user is allowed only one draft per record.

Omit ```draft=true``` from PUT request to replace a draft with a proper submission.

!!!note
     The id of a record changes each time it is saved as a draft, but the record UID remains the same.
