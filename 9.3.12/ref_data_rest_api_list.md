...
There are several optional query parameters that can affect the output.
...
When ```?draft=true``` is used, the response returns all records and drafts that the current user can access. For records not in the Start stage, the current draft version of the user is returned instead of the original record. A populated **draft_ownerid** attribute in the response indicates that the item is the user’s draft rather than the original record.

```?draft=true&flow_state=ST_Start``` reduces the query to records in the Start stage. If there is a draft, it will be returned in the response. This can be used to determine whether or not the current user has a draft in the Start stage.  

!!!note
	   A user can only have one draft in the Start stage
